...
uR_e = cell(d1,d2,neq);
uR_w = cell(d1,d2,neq);
fxR_n = cell(d1,d2,neq);
fxR_s = cell(d1,d2,neq);
fxR_e = cell(d1,d2,neq);
fxR_w = cell(d1,d2,neq);
fyR_n = cell(d1,d2,neq);
fyR_s = cell(d1,d2,neq);
fyR_e = cell(d1,d2,neq);
fyR_w = cell(d1,d2,neq);
...
[fx_s, fy_s] = flux_function_new(u_s,eq_type,radius,hx,hy,x_c,y_c-hy/2,pts_x,zeros(size(pts_y)));
...
for i=2:d1-1
    for j=2:d2-1
        for n=1:neq
            ...
            fxR_n{i,j,n} = fx_s{i,j+1,n};
            fxR_s{i,j,n} = fx_n{i,j-1,n};
            fxR_e{i,j,n} = fx_w{i+1,j,n};
            fxR_w{i,j,n} = fx_e{i-1,j,n};

            fyR_n{i,j,n} = fy_s{i,j+1,n};
            fyR_s{i,j,n} = fy_n{i,j-1,n};
            fyR_e{i,j,n} = fy_w{i+1,j,n};
            fyR_w{i,j,n} = fy_e{i-1,j,n};
        end
    end
end
...
for i=1:d1
        for n=1:neq
% Northmost
            uR_n{i,d2,n} = u_s{i,1,n};
            uR_s{i,d2,n} = u_n{i,d2-1,n};
            uR_e{i,d2,n} = u_w{i+1,d2,n};
            uR_w{i,d2,n} = u_e{i-1,d2,n};

            fxR_n{i,d2,n} = fx_s{i,1,n};
            fxR_s{i,d2,n} = fx_n{i,d2-1,n};
            fxR_e{i,d2,n} = fx_w{i+1,d2,n};
            fxR_w{i,d2,n} = fx_e{i-1,d2,n};

            fyR_n{i,d2,n} = fy_s{i,1,n};
            fyR_s{i,d2,n} = fy_n{i,d2-1,n};
            fyR_e{i,d2,n} = fy_w{i+1,d2,n};
            fyR_w{i,d2,n} = fy_e{i-1,d2,n};

% Southmost
            uR_n{i,1,n} = u_s{i,2,n};
            uR_s{i,1,n} = u_n{i,d2,n};
            uR_e{i,1,n} = u_w{i+1,1,n};
            uR_w{i,1,n} = u_e{i-1,1,n};

            fxR_n{i,1,n} = fx_s{i,2,n};
            fxR_s{i,1,n} = fx_n{i,d2,n};
            fxR_e{i,1,n} = fx_w{i+1,1,n};
            fxR_w{i,1,n} = fx_e{i-1,1,n};

            fyR_n{i,1,n} = fy_s{i,2,n};
            fyR_s{i,1,n} = fy_n{i,d2,n};
            fyR_e{i,1,n} = fy_w{i+1,1,n};
            fyR_w{i,1,n} = fy_e{i-1,1,n};
...
% Eastmost and Westmost cells
    for j=1:d2
        for n=1:neq
% Eastmost
            uR_n{d1,j,n} = u_s{d1,j+1,n};
            uR_s{d1,j,n} = u_n{d1,j-1,n};
            uR_e{d1,j,n} = u_w{1,j,n};
            uR_w{d1,j,n} = u_e{d1-1,j,n};

            fxR_n{d1,j,n} = fx_s{d1,j+1,n};
            fxR_s{d1,j,n} = fx_n{d1,j-1,n};
            fxR_e{d1,j,n} = fx_w{1,j,n};
            fxR_w{d1,j,n} = fx_e{d1-1,j,n};

            fyR_n{d1,j,n} = fy_s{d1,j+1,n};
            fyR_s{d1,j,n} = fy_n{d1,j-1,n};
            fyR_e{d1,j,n} = fy_w{1,j,n};
            fyR_w{d1,j,n} = fy_e{d1-1,j,n};

% Westmost
            uR_n{1,j,n} = u_s{1,j+1,n};
            uR_s{1,j,n} = u_n{1,j-1,n};
            uR_e{1,j,n} = u_w{2,j,n};
            uR_w{1,j,n} = u_e{d1,j,n};

            fxR_n{1,j,n} = fx_s{1,j+1,n};
            fxR_s{1,j,n} = fx_n{1,j-1,n};
            fxR_e{1,j,n} = fx_w{2,j,n};
            fxR_w{1,j,n} = fx_e{d1,j,n};

            fyR_n{1,j,n} = fy_s{1,j+1,n};
            fyR_s{1,j,n} = fy_n{1,j-1,n};
            fyR_e{1,j,n} = fy_w{2,j,n};
            fyR_w{1,j,n} = fy_e{d1,j,n};
        end
    end
...
% TODO: check the fact_bd factor...  Why is this not simply a scalar???  Why does it not apply to EW fluxes??
                for n=1:neq
                    flux_n{i,j,n} =  1/2*(fy_n{i,j,n}+fyR_n{i,j,n}).*fact_bd - alpha/2 * (uR_n{i,j,n} - u_n{i,j,n}) .* fact_bd; 
                    flux_s{i,j,n} = -1/2*(fy_s{i,j,n}+fyR_s{i,j,n}).*fact_bd - alpha/2 * (uR_s{i,j,n} - u_s{i,j,n}) .* fact_bd;
                    flux_e{i,j,n} =  1/2*(fx_e{i,j,n}+fxR_e{i,j,n}) - alpha/2 * (uR_e{i,j,n} - u_e{i,j,n});
                    flux_w{i,j,n} = -1/2*(fx_w{i,j,n}+fxR_w{i,j,n}) - alpha/2 * (uR_w{i,j,n} - u_w{i,j,n});
                end
            end
