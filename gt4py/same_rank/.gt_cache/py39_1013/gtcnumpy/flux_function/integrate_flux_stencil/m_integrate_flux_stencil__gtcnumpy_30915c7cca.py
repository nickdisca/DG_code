...
import m_computation__gtcnumpy_30915c7cca as computation
...
class integrate_flux_stencil____gtcnumpy_30915c7cca(StencilObject):
    """
    ...
    def __call__(
        self, w, fx, fy, phi_grad_x, phi_grad_y, rhs, domain=None, origin=None, validate_args=True, exec_info=None
    ):
        if exec_info is not None:
            exec_info["call_start_time"] = time.perf_counter()

        field_args = dict(w=w, fy=fy, fx=fx, rhs=rhs, phi_grad_x=phi_grad_x, phi_grad_y=phi_grad_y)
        parameter_args = dict()
        ...
        if exec_info is not None:
            ...
            if exec_info.setdefault("__aggregate_data", False):
                stencil_info = exec_info.setdefault("integrate_flux_stencil____gtcnumpy_30915c7cca", {})
