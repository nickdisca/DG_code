...
def fused_internal_stencils(
    ...
    with computation(PARALLEL), interval(...):
        ...
        fh_x = hu_qp * w 
        fh_y = hv_qp * w * cos_fact

        fhu_x = hu_qp / h_qp * w
        fhu_y = hu_qp * hv_qp / h_qp  * w 

        # fhv_x = hu_qp * hv_qp / h_qp * w / bd_det_x * determ
        # fhv_y = (hv_qp * hv_qp / h_qp + h_qp * h_qp * g / 2) * w * cos_fact / bd_det_x * determ

        rhs_h = phi_grad_x.T @ fh_x
        # rhs_hu = phi_grad_x.T @ fhu_x
        # rhs_hv = phi_grad_x.T @ fhv_x

        # rhs_h = phi_grad_x.T @ fh_x  + phi_grad_y.T @ fh_y
        # rhs_hu = phi_grad_x.T @ fhu_x + phi_grad_y.T @ fhu_y
        # rhs_hv = phi_grad_x.T @ fhv_x + phi_grad_y.T @ fhv_y
